...
class CodeBook : public vector<CodeElement>
{
	...
	CodeBook( int n ) : vector<CodeElement>(n) // Construct book of size n
	{
		t=0;
	}
};
...
uchar backgroundDiff( // return 0 => background, 255 => foreground
	...
	int* minMod_,          // Add this (possibly negative) number onto max level when determining whether new pixel is foreground
	int* maxMod_           // Subtract this (possibly negative) number from min level when determining whether new pixel is foreground
)
{
	int matchChannel;
