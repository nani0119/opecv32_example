...
void help(char *argv[]) {
  cout
      ...
      << "\n    directly below the source directory and are "
      ...
      << "\n    ../resource/19/stereoData/example_list.txt file, you can comment out "
         "lines"
      << "\n    there by starting them with #."
      << "\n"
      << "\nDefault Call (with parameters: board_w = 9, board_h = 6, list = "
         "../resource/19/stereoData/example_list.txt):"
      << "\n" << argv[0] << "\n"
      << "\nManual call:"
...
static void StereoCalib(const char *imageList, int nx, int ny,
  ...
  for (;;) {
    ...
    if (img.empty())
    {
        cout << "img data is empty:"<< buf << endl;
        break;
    }
    imageSize = img.size();
...
int main(int argc, char **argv) {
  ...
  const char *board_list = "../resource/19/stereoData/example_list.txt";
  if (argc == 4) {
    board_list = argv[1];
    board_w = atoi(argv[2]);
    board_h = atoi(argv[3]);
