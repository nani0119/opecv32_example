...
static void  update_mhi(const cv::Mat& img, cv::Mat& dst, int diff_threshold)
{
    ...
    cv::threshold(silh, silh, diff_threshold, 1, cv::THRESH_BINARY); // and threshold it
    ...
    for (i = -1; i < (int)regions.size(); i++) {
        ...
        count = norm(silh_roi, cv::NORM_L1);; // calculate number of points within silhouette ROI
        ...
        cv::line(img, center, cv::Point(cvRound(center.x + magnitude*cos(angle*CV_PI / 180)),
...
int main(int argc, char** argv)
{
    ...
    cap.open(RESOURCE_DIR"/17/motion.mkv");

    if (!cap.isOpened())
    {
        cout << "Could not initialize video capture" <<endl;
        return 0;
    }
    buf.resize(2);
    cv::Mat image, motion;
